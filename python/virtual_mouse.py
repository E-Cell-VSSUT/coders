...
while(1):
    ...
    if(len(conts)==2):
        ...
        mlocold = mouseloc
        #when double object are there then only drag or selection will be done
        openx,openy,openw,openh=cv.boundingRect(np.array([[[x1,y1],[x1+w1,y1+h1],[x2,y2],[x2+w2,y2+h2]]]))
        #squar the total object
        cv.rectangle(img,(openx,openy),(openx+openw,openy+openh),(255,0,0),2)
